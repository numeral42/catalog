<nav class="bg-orange-500" x-data="{open:false}">
      ...
      <div class="flex-1 flex items-center justify-center sm:items-stretch sm:justify-start">
        ...
        <div class="hidden sm:block sm:ml-6">
          <div class="flex space-x-4">
            ...
            {{-- <a href="#" class="bg-orange-900 text-white px-3 py-2 rounded-md text-sm font-medium" aria-current="page">Dashboard</a> --}}
        @foreach ($categories as $category)
            <a href="{{route('products.category', $category)}}" class="text-gray-300 hover:bg-gray-700 hover:text-white px-3 py-2 rounded-md text-sm font-medium">{{$category->name}}</a>
...
@auth
    ...
    {{--Boton notificación--}}
      <button type="button" class="bg-gray-800 p-1 rounded-full text-gray-400 hover:text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-white">
          ...
          <!-- <span class="sr-only">View notifications</span>s -->
...
{{--             <a href="{{route('index')}}" class="block px-4 py-2 text-sm text-gray-700" 
            role="menuitem" tabindex="-1" id="user-menu-item-0">Pruebas</a> --}}
      ...
      @can('admin.index')      
            ...
            <form method="POST" action="{{ route('logout') }}" x-data>
                ...
                onclick="event.preventDefault(); this.closest('form').submit();">Cerrar sessión</a>
...
@endauth
 ...
 -->
      @foreach ($categories as $category)
           <a href="{{route('products.category', $category)}}" class="text-gray-300 hover:bg-gray-700 hover:text-white block px-3 py-2 rounded-md text-base font-medium">{{$category->name}}</a>
